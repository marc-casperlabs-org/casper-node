...
impl EngineState<DbGlobalState> {
    ...
    pub fn migrate_state_root_to_rocksdb_if_needed(
        ...
        if !self.is_state_root_migrated(state_root)? {
            self.state
                .migrate_state_root_to_rocksdb(state_root, limit_rate)?;
            self.mark_state_root_migrated(state_root)?;
            Ok(true)
        } else {
            Ok(false)
        }
    ...
    fn mark_state_root_migrated(
        &self,
        state_root: Digest,
    ) -> Result<(), storage::error::db::Error> {
        self.state
            .rocksdb_store
            .rocksdb
            .store_migrated_state_root(&state_root.value())
    }
