...
impl DbGlobalState {
        ...
        while let Some(next_trie_key) = missing_trie_keys.pop() {
            ...
            match lmdb_txn.read(self.lmdb_trie_store.db, &next_trie_key.to_bytes()?)? {
                Some(value_bytes) => {
                    ...
                    total_tries += 1;
                    handle.write(rocksdb, &key_bytes, &value_bytes)?;
                }
                None => {
            ...
            let missing_descendents =
                self.missing_trie_keys(CorrelationId::new(), vec![next_trie_key], false)?;
            missing_trie_keys.extend(missing_descendents);
        }
        info!(
            "trie migration: total_bytes copied {}, total tries copied {}, total time micros {}",
            total_bytes,
            total_tries,
    ...
    /// Copy all tries from lmdb to rocksdb using a raw lmdb cursor. This is useful for test
    /// fixtures only, and is not intended to be used by the node.
    pub fn copy_all_tries_from_lmdb_to_rocksdb(&self) -> Result<(), error::Error> {
        use lmdb::{Cursor, Transaction as LmdbTransaction};

        let lmdb_txn = self.lmdb_environment.env().begin_ro_txn()?;
        let mut cursor = lmdb_txn.open_ro_cursor(self.lmdb_trie_store.db)?;
        ...
        for (key, value) in cursor.iter() {
            let rocksdb = self.rocksdb_store.rocksdb.clone();
            handle.write(rocksdb, key, value)?;
        }
